...
for i = 1:fileCount
   if(~bluesFileList(i).isdir && ~strcmpi(bluesFileList(i).name, 'cmds') && ~strcmpi(bluesFileList(1),'.')) 
       cmd = ['./tempo ', fullfile(bluesDirectory, bluesFileList(i).name)];
       [~, cmdout] = system(cmd);
...
for i = 1:fileCount
   if(~countryFileList(i).isdir && ~strcmpi(countryFileList(i).name, 'cmds') && ~strcmpi(countryFileList(1),'.')) 
       cmd = ['./tempo ', fullfile(countryDirectory, countryFileList(i).name)];
       [~, cmdout] = system(cmd);
...
for i = 1:fileCount
   if(~discoFileList(i).isdir && ~strcmpi(discoFileList(i).name, 'cmds') && ~strcmpi(discoFileList(1),'.')) 
       cmd = ['./tempo ', fullfile(discoDirectory, discoFileList(i).name)];
       [~, cmdout] = system(cmd);
...
for i = 1:fileCount
   if(~hiphopFileList(i).isdir && ~strcmpi(hiphopFileList(i).name, 'cmds') && ~strcmpi(hiphopFileList(1),'.')) 
       cmd = ['./tempo ', fullfile(hiphopDirectory, hiphopFileList(i).name)];
       [~, cmdout] = system(cmd);
...
for i = 1:fileCount
   if(~metalFileList(i).isdir && ~strcmpi(metalFileList(i).name, 'cmds') && ~strcmpi(metalFileList(1),'.')) 
       cmd = ['./tempo ', fullfile(metalDirectory, metalFileList(i).name)];
       [~, cmdout] = system(cmd);
...
for i = 1:fileCount
   if(~popFileList(i).isdir && ~strcmpi(popFileList(i).name, 'cmds') && ~strcmpi(popFileList(1),'.')) 
       cmd = ['./tempo ', fullfile(popDirectory, popFileList(i).name)];
       [~, cmdout] = system(cmd);
...
for i = 1:fileCount
   if(~reggaeFileList(i).isdir && ~strcmpi(reggaeFileList(i).name, 'cmds') && ~strcmpi(reggaeFileList(1),'.')) 
       cmd = ['./tempo ', fullfile(reggaeDirectory, reggaeFileList(i).name)];
       [~, cmdout] = system(cmd);
...
for i = 1:fileCount
   if(~rockFileList(i).isdir && ~strcmpi(rockFileList(i).name, 'cmds') && ~strcmpi(rockFileList(1),'.')) 
       cmd = ['./tempo ', fullfile(rockDirectory, rockFileList(i).name)];
       [~, cmdout] = system(cmd);
...
for i=1:100
    fprintf(fid, '%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f \n', tempo(1,i), tempo(2,i), tempo(3,i), tempo(4,i), tempo(5,i), tempo(6,i), tempo(7,i), tempo(8,i));
end
fclose(fid);
end
